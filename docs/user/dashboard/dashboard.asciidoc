...
Panels display your data in charts, tables, maps, and more, which allow you to compare your data side-by-side to identify patterns and connections. Dashboards support several types of panels that display your data.
...
Area charts are similar, but the space below the line is shaded.
...
a| *Machine Learning*
Display the results from <<xpack-ml-anomalies,anomaly detection jobs>> and display <<xpack-ml-anomalies,anomaly charts>> from the *Anomaly Explorer*.

| image:

a| *Observability*
Display a <<logs-app,log stream>> table of live streaming logs.

| image:

| *Tools*
Add interactive filters with <<add-controls,*Controls*>> panels.

| image:

| *Text*
Add context to your panels with <<add-text,*Text*>>.

| image:

| *Images*

Personalize your dashboard with custom <<add-image,images>>.
