...
Save your search so you can use it later, generate a CSV report, or use it to create visualizations to display on dashboards and Canvas workpads.
Saving a search saves the query text, filters, and current view of *Discover*, including the columns selected in
the document table, the sort order, and the {data-source}. When you add visualizations of search results to dashboards, the results are not aggregated.
...
if you click **Visualize**, your data appears in a map.
+
[role="screenshot"]
